...
uint16_t Adafruit_ADS1015::readRegister(uint8_t i2cAddress, uint8_t reg) {
  ...
  return  this->readDoubleByte(reg);
}
...
*/
/**************************************************************************/
Adafruit_ADS1015::Adafruit_ADS1015(uint8_t i2cAddress) : I2CBase(I2C_BUS_NAME, i2cAddress)
{
...
uint16_t Adafruit_ADS1015::readADC_ContineousSampleAvg(uint8_t channel, uint16_t count) {
  if (channel > 3)
  {
    return 0;
  }
  ...
  // Shift 12-bit results right 4 bits for the ADS1015
  uint32_t tmp = 0;
  readRegister(m_i2cAddress, ADS1015_REG_POINTER_CONVERT);
  for (uint16_t i = 0; i < count; i++) {
    tmp += readRegister(m_i2cAddress, ADS1015_REG_POINTER_CONVERT) >> m_bitShift;
  }
  return tmp / count;
}
